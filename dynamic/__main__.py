...
Examples::
    ...
    python -m dynamic
    python -m dynamic --reform taper --elasticity 0.17
    python -m dynamic --reform rate10 --static
"""
...
from .figures import RESULTS_DIR, fig_notch_fit, fig_reform_distribution
from .model import (
    ELASTICITIES,
    E_HEADLINE,
    build_reforms,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="python -m dynamic")
    ...
    print("\nLoading reform-costing dataset (PolicyEngine_VATLab) ...")
    df = load_reform_data()
    ...
    for rname, (sched, label, _tgt) in reforms.items():
