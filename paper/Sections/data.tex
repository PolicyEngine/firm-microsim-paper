...
the optimizer, because the generator draws a common input-share distribution
with sector shifts rather than calibrating the input/output tax structure by
sector. The problem is solved by gradient descent, and the symmetric loss keeps
targets of very different magnitudes on a common scale.
...
pounds of the threshold, however, is not pinned by any target: it is an
equilibrium of the weight optimiser, and in the corrected calibration the local
density steps \emph{up} across \pounds85{,}000 (about $5{,}400$ firms per
\pounds1{,}000 just below against $7{,}300$ just above)---the opposite of the
bunching pattern in administrative data \citep{obr2023vat,liuetal2021}. This is
the central data caveat of the paper: no sub-band feature of band-calibrated
synthetic data, in either direction, carries behavioural information
...
\pounds1{,}000 bins), 2023--24. The band-average density steps down across the
\pounds85{,}000 threshold and again at \pounds150{,}000; both are edges of the
HMRC calibration bands, and the within-band shape between them is an optimiser
equilibrium, not data (Section~\ref{sec:bunching}).}
