...
(about 2.7 million); the full private-sector business population including
the 2.9 million businesses registered for neither is about 5.5 million (DBT
business population estimates, start of 2024).
...
Four findings follow. First, the static pipeline prices level, shape, and
rate reforms on one basis; its anchor---the April 2024 rise from
\pounds85{,}000 to \pounds90{,}000, whose mechanical cost profile lands
within 30\% of HMRC's published costing in each of the four release years
(within 10\% in 2025--26 and 2026--27) and 36\% above it in the sign-flip
year, with no retention or behavioural parameter tuned---is a consistency check rather than an out-of-sample validation, since
the population is calibrated to HMRC aggregates (Section~\ref{sec:static}). Second, the shape and rate reforms act on the
notch's distortion in a way a level change does not: the notch creates an
exact \pounds21{,}250 dominated region for a firm bearing no unreclaimed input
VAT---the upper bound; once unregistered firms bear VAT on inputs the region is
firm-specific and vanishes for the 73\% of near-threshold firms with input
shares above one half---which a rise relocates and widens, a
reduced-rate band splits across two notches, and only a continuous taper
removes---and the taper's band width is a property of its shape, not a
requirement of removal (Section~\ref{sec:model}). Third, conditional on the
assumed elasticity, intensive-margin responses barely move the costings
(Section~\ref{sec:behavioural}). Fourth, the synthetic data carry a
transparency lesson: their near-threshold excess mass is inherited from
calibration targets or band edges, is unstable across specifications, and
cannot validate a bunching estimator or identify behaviour
(Section~\ref{sec:bunching}); the behavioural evidence remains the
administrative-data work of \citet{liuetal2021}. Table~\ref{tab:summary}
collects the headline estimates.
...
data. The fourth strand is the theory of size-based tax thresholds: the optimal
VAT threshold of \citet{keenmintz2004}, thresholds as partitions of compliance
in \citet{kanburkeen2014}, and optimal remittance with firm-level
administrative costs in \citet{dharmapala2011}, together with the structural
literature on size-dependent policy \citep{garicano2016,gourioroys2014}; the
Mirrlees Review's VAT chapter frames the UK debate \citep{crawfordkeensmith2010}.
The present paper makes no comparable structural claim. Firm-level tax
microsimulation has precedents in corporate-tax models such as the German
BizTax model \citep{fossenbach2008}; to my knowledge no open firm-level model
of the UK VAT threshold exists, which is the gap this paper fills.
