...
intermediate build it also produced a large ($E\approx196{,}000$),
...
$E=5{,}498$ weighted firms and $b_{\mathrm{LLAT}}=5.357$ on the
...
to about 20{,}800 firms and the headline fitted missing mass above the
...
threshold the same estimator returns $E=10{,}273$ and
$b_{\mathrm{LLAT}}=4.971$, an excess mass produced entirely by the
...
estimator reads the target-inherited excess back as $E = 5{,}498$
($b_{\mathrm{LLAT}} = 5.357$). This is not behavioural:
...
$b_{\mathrm{LLAT}}=5.357$ is not a second estimate of their parameter: it is
