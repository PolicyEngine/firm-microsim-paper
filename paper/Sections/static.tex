...
the VAT net), and baseline voluntary-registration flag $r_i$ (one for firms
registered below the data-year threshold). A firm remits VAT only once
registered. Under a counterfactual threshold $T$ an in-scope firm is
registered when its turnover is at or above $T$; a firm registered at the
data-year threshold stays registered until its turnover falls below the
deregistration threshold $T-g$, where $g=\pounds2{,}000$ is the statutory
gap; voluntary registrants stay registered; out-of-scope firms never remit.
Total VAT revenue at threshold $T$ is therefore
\[
  R(T) \;=\; \sum_i w_i\, v_i \,
  \mathbf{1}\bigl\{\, s_i\,[\,y_i \ge T \;\lor\; (m_i \land y_i \ge T-g)\,]
  \;\lor\; r_i \,\bigr\},
\]
with $m_i$ the data-year mandatory-registration flag. Three conventions follow
and are worth stating. First, because voluntary registrants already remit, a
...
can leave the net. The profile's middle years differ in shape: once the
counterfactual threshold reaches \pounds87{,}000 and \pounds89{,}000, almost
no firm registered at \pounds85{,}000 sits between it and the
\pounds88{,}000 deregistration point, so the static model's impact falls
towards zero in 2027--28 where HMRC's $-$50m reflects a flow of new
businesses that would have registered under the lower threshold---an
entry margin a fixed cross-section cannot represent. Three sensitivities
...
\pounds88{,}000 deregistration threshold; its 2027--28 impact is near zero
because a fixed cross-section has no entry margin. Because the microdata are calibrated
