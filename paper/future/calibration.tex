...
region is the exact Kleven--Waseem width, the marginal buncher is evaluated in the
iso-elastic representation with the behavioural elasticity
(Section~\ref{ssec:buncher}), and the schedule-reform revenue counterfactuals turn
on the schedule and turnover, not on the precise value of $\alpha$.}
...
turnover elasticity. \emph{This is the elasticity I carry into the policy
analysis.} Alongside it I report the descriptive bunching ratio $b=0.061$
...
(CES-wedge) elasticity $\sigma\approx5.10$ (SE $0.12$), which reads the full
excess span as a response; it is not comparable to the local turnover elasticity
and is not used in the policy analysis. See Appendix~\ref{app:inference}.}

The mass-conservation constraint (Appendix~\ref{app:inference}) raises $b$ (from
$0.054$) and the excess mass $E$ (from $8{,}179$ to $9{,}160$) relative to the
unconstrained fit, and forces excess and missing mass into agreement ($E=9{,}160$
versus $\Delta_{R}=9{,}938$, against $8{,}179$ versus $17{,}792$ unconstrained);
the implied displaced share $\Pi=0.220$ is robust even where the wedge level is
not. The sigmoid steepness $k$ does not affect these data-side magnitudes but
governs the width of the structural schedule's dominated region; I set
$k=0.001$, whose implied band ($\pounds19.7$k) is closest to the exact
\citet{klevenwaseem2013} notch region $T^{*}\tau/(1-\tau)=\pounds21.25$k.
