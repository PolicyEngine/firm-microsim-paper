% SUPERSEDED (pre-correction draft, not built into main.pdf): numbers here
% predate the net-liability correction and are retracted; see Sections/.
...
\paragraph{Revenue.} Against a baseline VAT base of \pounds183.6bn (the
\pounds85{,}000-notch year), the taper reduces revenue by \pounds336m on a
...
iso-elastic intensive-margin simulator gives a lower-bound behavioural cost of
about \pounds156m at the headline $e=0.17$, but this omits the taper's
marginal-rate channel and is not a full policy estimate. The reform affects about
$123{,}100$ weighted firms sitting in the \pounds85{,}000--\pounds105{,}000 band.

\paragraph{Sensitivity.} The static revenue figure turns only on the taper schedule
and observed turnover. The behavioural taper number is conditional on the assumed
elasticity and deliberately not headlined: the iso-elastic model captures the
base-broadening from a lower average rate but not the downward response induced by
the taper's rising marginal rate. I therefore rest the distributional argument on
the exact, model-free analytic mechanism above rather than on a simulated
magnitude.
...
\paragraph{Revenue.} Against the same \pounds183.6bn baseline, the banded reduced
rate reduces revenue, \emph{statically}, by \pounds343m at
$\tau_{\text{low}}=10\%$ and by \pounds171m at $\tau_{\text{low}}=15\%$, with
about $123{,}100$ firms in the band facing the lower rate. I report these static
figures as the robust object. Conditional on the headline elasticity $e=0.17$, the
iso-elastic intensive-margin simulator gives behavioural costs of \pounds273m and
\pounds135m, respectively; these are reported as sensitivity results rather than
...
essentially none of it at a static cost of \pounds508m. A reduced rate
...
\pounds21{,}250 zone is removed---and at a static cost (\pounds336m) below that of
the threshold move.
...
base \pounds183.6bn, 2023--24 data year). All four reforms are costed against
this common \pounds85{,}000-notch / \pounds183.6bn base. Each reform pulls a different lever---location,
...
\midrule
Raise threshold to \pounds100{,}000 & Location & $-508$ & \pounds21{,}250 (relocated) & $\approx\pounds0$ \\
Reduced rate $15\%$ & Rate & $-171$ & \pounds15{,}000 & \pounds6{,}250 \\
Reduced rate $10\%$ & Rate & $-343$ & \pounds9{,}444 & \pounds11{,}806 \\
Graduated taper & Shape & $-336$ & \pounds0 (no notch) & \pounds21{,}250 (whole zone) \\
\bottomrule
\end{tabular}
\end{table}

The ranking carries a clear message: \emph{schedule reforms dominate
threshold-location moves on efficiency}. A location move is the most expensive
option on the menu yet buys almost no distortion reduction---it merely transports
...
(\(-\)\pounds343m) costs almost exactly what the taper costs
(\(-\)\pounds336m), because a $0\%\to20\%$ linear phase-in averages roughly $10\%$
