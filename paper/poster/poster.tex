...
\begin{column}{\colwidth}
  ...
  \begin{figure}
    \centering
    {\itshape All UK firms by turnover band --- ONS, 2023--24}\\[0.4em]
  ...
  \begin{exampleblock}{Data methodology}
    ...
    input share $x_i=\rho_i y_i$ so value added $v_i=y_i-x_i$ is the net VAT
    base. Firm weights $w_i=e^{\theta_i}$ are fit by gradient descent to HMRC
    + ONS aggregates. Crucially, the threshold is a \emph{generator
    parameter}: every counterfactual --- a higher cutoff, a taper, a reduced
    rate --- \hl{regenerates} a fresh, internally consistent population, and
    the whole file is \hl{released openly}.
  \end{exampleblock}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Threshold sweep (2025--26)}
    ...
    From a \pounds90k base (\hl{\pounds203.8bn}), the static cost is roughly
    \hl{linear}, $\approx$\pounds150--220m per \pounds5k step --- e.g.\
    \hl{$-$\pounds374m} to raise to \pounds100k, \hl{$+$\pounds665m} to lower
    to \pounds70k.
  \end{block}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Reform menu}
    \leadin{\flow Cost and distortion on a common \pounds85k / \pounds183.6bn base.}
    \centering
    ...
    \begin{tabular}{@{}l c r l@{}}
      ...
      Taper \pounds85--105k & Shape & $-$\pounds336m & {\color{pedarkteal}\bfseries removed}\\
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Behavioural layer}
    ...
    identified on synthetic data); sweep $e\in\{0.05,0.17,0.32\}$. At the
    midpoint $e=0.17$, each static cost falls as firms broaden the base:
    raising to \pounds100k from \pounds508m to \hl{\pounds292m}; the 10\% band
    \pounds343m to \hl{\pounds273m}; the 15\% band \pounds171m to
    \hl{\pounds135m}. A larger $e$ makes every reform \hl{cheaper}. The taper
    is \emph{excluded} --- its base-broadening and marginal-rate channels
    cannot be netted in the iso-elastic cost. Only $e=0.05$ has an external
    anchor, so this is a \hl{sensitivity range}, not a forecast.
    \begin{figure}
      \centering
